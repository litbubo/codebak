...
u8 DHT11_Check(void)
{
    ...
    {
        while (GPIO_ReadInputDataBit(GPIO_DHT11, IO_DHT11) == 0 && (retry < 200))
            retry++;
    }
    delay(40);
    delay(40);
...
uint8_t DHT_ByteRead(unsigned char *dat)
{
    ...
    for (y = 0; y < 5; y++)
    {
        ...
        {
            while (GPIO_ReadInputDataBit(GPIO_DHT11, IO_DHT11) == 0 && m < 200)
                m++;
            delay(30);
            if (GPIO_ReadInputDataBit(GPIO_DHT11, IO_DHT11) == 1)
                temp |= mask;
                ...
                temp &= (~mask);
            while (GPIO_ReadInputDataBit(GPIO_DHT11, IO_DHT11) == 1 && n < 200)
                n++;
        }
        *(dat + y) = temp;
        temp = 0;
