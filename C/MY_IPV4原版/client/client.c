...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    while (1)
    {
        ...
        c = getopt_long(argc, argv, "P:M:h:H", argarr, &index);
        if (c < 0)
            break;
        switch (c)
        {
    ...
    if (setsockopt(sd, SOL_SOCKET, SO_RCVBUF, &receive_buf_size, sizeof(receive_buf_size)) < 0)
    {
        perror("setsockopt()");
        exit(1);
    }
    ...
    else // parent
    {
        ...
        */
        ...
        while (1)
        {
            ...
            {
                fprintf(stdout, "Accept massage:%d, size:%ld recived.\n", msg_channel->chnid, len - sizeof(chnid_t));
