// 45_给共享智能指针shared_ptr指定删除器.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	shared_ptr<Test> ppp(new Test[5]{ 1,2,3,4,5 },default_delete<Test[]>());
	auto myp = ppp.get();
	myp[2].print_num();
	ppp.reset();

	shared_ptr<Test> ppp2(new Test[5]{ 6,7,8,9,10 }, [](Test *t)
		{
			cout << "diy" << endl;
			delete[] t;
		});
